...
#include "add_numbers23.comp.spv"
    ;
...
int main_entry(const entry::entry_data* data) {
  ...
  const uint32_t kNumStorageBuffers = 24;  // The total number of storage buffers
  ...
  containers::vector<VkDescriptorSetLayoutBinding> bindings(
      kNumStorageBuffers, data->root_allocator);
  ...
  auto compute_descriptor_set = containers::make_unique<vulkan::DescriptorSet>(
      data->root_allocator, app.AllocateDescriptorSet(bindings));
  ...
  containers::vector<containers::vector<VkDescriptorSetLayoutBinding>> layouts(
      data->root_allocator);
  layouts.push_back(bindings);

  // Create pipeline
  auto compute_pipeline_layout =
      containers::make_unique<vulkan::PipelineLayout>(
          data->root_allocator, app.CreatePipelineLayout(layouts));
              ...
              "main"));
  {
    ...
    std::vector<uint32_t> initial_buffer_values(kBufferElements, 1);
    for (size_t i = 0; i < kNumStorageBuffers; ++i) {
    ...
    cmd_buf->vkCmdDispatch(cmd_buf, kBufferElements / 128, 1, 1);
    ...
    std::vector<uint32_t> output =
        vulkan::GetHostVisibleBufferData(&*storage_buffers[kOutputBuffer]);
